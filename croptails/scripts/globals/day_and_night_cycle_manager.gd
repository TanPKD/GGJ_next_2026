...
func set_initial_time() -> void:
	var initial_total_minutes: int = initial_day * MINUTES_PER_DAY \
		+ (initial_hour * MINUTES_PER_HOUR) \
		+ initial_minute

	# 把“分钟”转换成 time 使用的单位
	time = float(initial_total_minutes) * GAME_MINUTE_DURATION

	# 关键：把当前检测值同步到初始值，避免开局误触发“新的一天”
